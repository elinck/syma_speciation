...
def IIMi(params, ns):
    ...
    fsi.integrate(nu_func, T1, dt_fac=0.01, m=np.array([[0, 0], [0, 0]]))
    fs2=P*fsi+(1-P)*fs
    return fs
...
for i in range(25):
        ...
        print(roundedparams)
        IIMimod=open('IIMi_modelparams.txt','a')
        IIMimod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
            ...
            str(L)+'\n')
        IIMimod.close()
        IIMireal=open('IIMi_realparams.txt','a')
        IIMireal.write(str(nu1_0)+'\t'+
            str(nu2_0)+'\t'+
            str(nu1)+'\t'+
            str(nu2)+'\t'+
            str(T0)+'\t'+
