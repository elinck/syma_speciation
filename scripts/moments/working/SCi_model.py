...
def SCi(params, ns):
    ...
    fsi.integrate([nu1,nu2], T0, dt_fac=0.01, m=np.array([[0, 0], [0, 0]]))
...
for i in range(25):
        ...
        print(roundedparams)
        SCimod=open('SCi_modelparams.txt','a')
        SCimod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
            ...
            str(L)+'\n')
        SCimod.close()
        SCireal=open('SCi_realparams.txt','a')
        SCireal.write(str(nu1_0)+'\t'+
            str(nu2_0)+'\t'+
            str(nu1)+'\t'+
            str(nu2)+'\t'+
            str(T0)+'\t'+
