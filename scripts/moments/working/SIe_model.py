...
def SIe(params, ns):
    nu1_0,nu2_0,nu1,nu2,T0,T1,hrf = params
    ...
    fs = moments.Spectrum(sts)
    fs = moments.Manips.split_1D_to_2D(fs, ns[0], ns[1])
    fs.integrate([nu1_0*hrf,nu2_0*hrf], T0, dt_fac=0.01, m=np.array([[0, 0], [0, 0]]))
    fs.integrate(lnu_func, T1, dt_fac=0.01, m=np.array([[0, 0], [0, 0]]))
    return  fs

func=SIe
upper_bound = [200,200,200,200,200,200,1]
lower_bound = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3]
...
for i in range(25):
        ...
        hrf=poptg[6]
        fst=data.Fst()
        realparams=[Nref,nu1_0,nu2_0,nu1,nu2,T0,T1,hrf,fst]
        roundedparams=['%.3f' % e for e in realparams]
        print(['Nref','nu1_0','nu2_0','nu1','nu2','T0','T1','hrf','Fst'])
        print(roundedparams)
        SIemod=open('SIe_modelparams.txt','a')
        SIemod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
            ...
            str(L)+'\n')
        SIemod.close()
        SIereal=open('SIe_realparams.txt','a')
        SIereal.write(str(nu1_0)+'\t'+
            str(nu2_0)+'\t'+
            str(nu1)+'\t'+
            str(nu2)+'\t'+
            str(T0)+'\t'+
