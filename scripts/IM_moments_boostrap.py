...
top_params=pd.read_csv("/media/burke/bigMac/ethan/moments_revision/IM_modelparams_v4.txt",sep="\t",header=None)
top_params=top_params.sort_values(8,ascending=False)
...
f=open("/media/burke/bigMac/ethan/moments_revision/IM_realparams_boots_v4.txt",'a')
f.write(out1)
f.close()
out2="\t".join(map(str,poptg))+"\t"+str(ll_model)+"\t"+str(theta)+"\n"
f=open("/media/burke/bigMac/ethan/moments_revision/IM_modelparams_boots_v4.txt",'a')
f.write(out2)
f.close()
