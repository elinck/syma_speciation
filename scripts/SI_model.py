...
func=SI
upper_bound = [200,200,200,200,200,200]
...
for i in range(25):
        ...
        T1=poptg[5]*2*Nref
        fst=data.Fst()
        realparams=[Nref,nu1_0,nu2_0,nu1,nu2,T0,T1,fst]
        roundedparams=['%.3f' % e for e in realparams]
        print(['Nref','nu1_0','nu2_0','nu1','nu2','T0','T1','Fst'])
        print(roundedparams)
        SImod=open('SI_modelparams.txt','a')
        SImod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
            ...
            str(L)+'\n')
        SImod.close()
        SIreal=open('SI_realparams.txt','a')
        SIreal.write(str(nu1_0)+'\t'+
            str(nu2_0)+'\t'+
            str(nu1)+'\t'+
            str(nu2)+'\t'+
            str(T0)+'\t'+
