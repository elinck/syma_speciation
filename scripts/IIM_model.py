...
func=IIM
upper_bound = [200,200,200,200,200,200,200,200]
lower_bound = [1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3]
...
for i in range(25):
        ...
        fst=data.Fst()
        realparams=[Nref,nu1_0,nu2_0,nu1,nu2,T0,T1,m12,m21,fst]
        roundedparams=['%.3f' % e for e in realparams]
        print(['Nref','nu1_0','nu2_0','nu1','nu2','T0','T1','m12','m21','Fst'])
        print(roundedparams)
        IIMmod=open('IIM_modelparams.txt','a')
        IIMmod.write(str(poptg[0])+'\t'+
            str(poptg[1])+'\t'+
            str(poptg[2])+'\t'+
            str(poptg[3])+'\t'+
            str(poptg[4])+'\t'+
            ...
            str(L)+'\n')
        IIMmod.close()
        IIMreal=open('IIM_realparams.txt','a')
        IIMreal.write(str(nu1_0)+'\t'+
            str(nu2_0)+'\t'+
            str(nu1)+'\t'+
            str(nu2)+'\t'+
            str(T0)+'\t'+
